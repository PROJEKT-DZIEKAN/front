...
export const useChat = () => {
  ...
  // NOWA FUNKCJA: Rozpocznij chat z użytkownikiem (dla adminów)
  const startChatWithUser = useCallback(async (targetUserId: number): Promise<Chat | null> => {
    ...
    if (mockMode) {
      console.log('📱 Using mock mode for startChatWithUser');
      const newChat: Chat = {
        id: Math.max(...chats.map(c => c.id), 0) + 1,
        userAId: user!.id,
    ...
    return chat;
  }, [user, mockMode, chats, getOrCreateChat, loadHistory]);
  ...
  return {
    ...
    getOrCreateChat,
    startSupportChat,
    startChatWithUser, // NOWA FUNKCJA
    canChatWith,
    hasAccessToChat,
    findAvailableAdmin,
    mockMode
